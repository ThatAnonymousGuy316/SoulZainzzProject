...
import swift.Script as SwiftScript;

using StringTools;

class FunkinSwift {
	public var script:SwiftScript;
	...
	public function set(name:String, arg:Dynamic){
		if (script != null){
			script.set(name, arg);
		}
	}
...
class FunkinHScript {
	public var instanceScript:Iris;

	public function new(Script:String){
		final rules:RawIrisConfig = {name: haxe.io.Path.withoutDirectory(Script), autoRun: false, autoPreset: true};
		...
		if (FileSystem.exists(Script)){
			...
			for (name in ["onCreate", 'onLoad', 'new', 'main']){
				call(name);
			}
		}
	}
	...
	public function call(name:String, ?args:Array<Dynamic>){
		if (instanceScript != null){
			if (instanceScript.exists(name)){
				instanceScript.call(name, args);
			}
		}
	}
	...
	public function set(name:String, arg:Dynamic){
		if (instanceScript != null){
			instanceScript.set(name, arg);
		}
	}

	public function destroy(){
		instanceScript.destroy();
	}
}
...
class FunkinLua {
	...
	public function new(script:String) {
		...
		Lua_helper.add_callback(lua, "windowTitle", function(daName:String){
            Lib.application.window.title = daName;
        });
		Lua_helper.add_callback(lua, "windowIcon", function(daName:String){
...
class HScript
{
	...
	{
		interp = new Interp();
		new HScriptPreset(interp.variables);
		interp.variables.set('_script_', this);
	}
...
class HScriptPreset {
	public function new(instance:Dynamic){
		instance.set('FlxG', FlxG);
		instance.set('FlxSprite', FlxSprite);
		instance.set('FlxCamera', FlxCamera);
		instance.set('FlxTimer', FlxTimer);
		instance.set('FlxTween', FlxTween);
		instance.set('FlxEase', FlxEase);
		instance.set('PlayState', PlayState);
		for (variable in ['state', 'game']){
			instance.set(variable, flixel.FlxG.state);
		}
		instance.set('subState', flixel.FlxG.state.subState);
		instance.set('Paths', Paths);
		instance.set('Conductor', Conductor);
		instance.set('ClientPrefs', ClientPrefs);
		instance.set('Character', Character);
		instance.set('Alphabet', Alphabet);
		instance.set('CustomSubstate', CustomSubstate);
		#if (!flash && sys)
		instance.set('FlxRuntimeShader', FlxRuntimeShader);
		#end
		instance.set('ShaderFilter', openfl.filters.ShaderFilter);
		instance.set('FlxColor', CustomFlxColor);
		...
		instance.set('FlxGifSprite', flxgif.FlxGifSprite);
		instance.set('FlxGroup', flixel.group.FlxGroup);
		...
		instance.set("AttachedSprite", AttachedSprite);
		instance.set("AttachedText", AttachedText);

		instance.set("Note", Note);
		instance.set("NoteSplash", NoteSplash);
		instance.set("StrumNote", StrumNote);

		instance.set("BGSprite", BGSprite);
		instance.set("SpriteFromSheet", SpriteFromSheet);

		instance.set("add", FlxG.state.add);
		...
		if (flixel.FlxG.state == PlayState.instance){
			...
			instance.set("addBehindGF", PlayState.instance.addBehindGF);
			instance.set("addBehindBF", PlayState.instance.addBehindBF);
			instance.set("addBehindDad", PlayState.instance.addBehindDad);
			instance.set('setVar', function(name:String, value:Dynamic)
			{
				PlayState.instance.variables.set(name, value);
			});
			instance.set('getVar', function(name:String)
			{
				var result:Dynamic = null;
				if(PlayState.instance.variables.exists(name)) result = PlayState.instance.variables.get(name);
				return result;
			});
			instance.set('removeVar', function(name:String)
			{
