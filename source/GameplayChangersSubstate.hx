...
class GameplayChangersSubstate extends MusicBeatSubstate
{
	...
	public static var maxValue:Float = 1000000000;
	...
	function getOptions()
	{
		...
		var option:GameplayOption = new GameplayOption('Crash On Miss', 'crashonmiss', 'bool', false);
		optionsArray.push(option);

		var option:GameplayOption = new GameplayOption('Crash On Sick', 'crashonsick', 'bool', false);
		optionsArray.push(option);
	}
									...
									if (curOption.name == "Scroll Type")
									{
										...
										if (oOption != null)
										{
											if (curOption.getValue() == "constant")
											{
												...
												oOption.maxValue = maxValue;
											}
											else
											{
												oOption.displayFormat = "%vX";
												oOption.maxValue = maxValue;
												if(oOption.getValue() > maxValue) oOption.setValue(maxValue);
											}
			...
			if(controls.RESET)
			{
				for (i in 0...optionsArray.length)
				{
					...
					if(leOption.name == 'Scroll Speed')
					{
						...
						leOption.maxValue = maxValue;
						if(leOption.getValue() > maxValue)
						{
							leOption.setValue(maxValue);
						}
						updateTextFrom(leOption);
					}
					leOption.change();
