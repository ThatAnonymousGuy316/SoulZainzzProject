...
class OptionsState extends MusicBeatState
{
	...
	override function create() {
		if (Redirects.OptionsState != null && Redirects.OptionsState != ''){
			MusicBeatState.switchState(new HXState(Redirects.OptionsState));
		}
...
class PreferencesSubstate extends MusicBeatSubstate
{
		...
		if(usesCheckbox) {
			if(controls.ACCEPT && nextAccept <= 0) {
				switch(options[curSelected]) {
					...
					case 'Round Accuracy':
						ClientPrefs.RoundLeDecimal = !ClientPrefs.RoundLeDecimal;
			...
			if(controls.UI_LEFT || controls.UI_RIGHT) {
				...
				var addFloat:Float = controls.UI_LEFT ? -0.05 : 0.05;
				if(holdTime > 0.5 || controls.UI_LEFT_P || controls.UI_RIGHT_P)
				switch(options[curSelected]) {
					case 'Scroll Speed':
						ClientPrefs.scrollSpeedSetting += addFloat;
						ClientPrefs.scrollSpeedSetting = FlxMath.roundDecimal(ClientPrefs.scrollSpeedSetting, 2);

						if (ClientPrefs.scrollSpeedSetting < 0)
							ClientPrefs.scrollSpeedSetting = 0;
						else if (ClientPrefs.scrollSpeedSetting > 12)
							ClientPrefs.scrollSpeedSetting = 12;
	...
	function changeSelection(change:Int = 0)
	{
		...
		switch(options[curSelected]) {
			case 'Round Accuracy':
				daText = "Rounds The Accuracy on the score text removing the decimal";
			case 'Scroll Speed':
		...
		for (i in 0...checkboxArray.length) {
			...
			if(checkbox != null) {
				...
				switch(options[checkboxNumber[i]]) {
					case 'Round Accuracy':
						daValue = ClientPrefs.RoundLeDecimal;
					case 'FPS Counter':
		...
		for (i in 0...grpTexts.members.length) {
			...
			if(text != null) {
				...
				switch(options[textNumber[i]]) {
					case 'Scroll Speed':
						daText = '< ' + ClientPrefs.scrollSpeedSetting + ' >';
					case 'Framerate':
